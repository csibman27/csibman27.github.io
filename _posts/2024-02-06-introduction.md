...
Create a single web page, with name, project, photo, and additional links for the project which will include some or all of the following:
    ...
    **Abstract**/Summary text 100 words max, for use in showcase handbook.
    LCManager is a web application for administrators or technicians to manage, plan, and execute servers and software lifecycles. It solves some of the problems system administrators are facing in an on-premises server room every day such as, the visibility of all servers lifecycle stage and age. The software would bring in an oragnized catalog which would indicate all the factors when the server needs to be updated or disposed or any other action.
